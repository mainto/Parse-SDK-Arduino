...
void WebSocketClient::sendEncodedData(char *str, uint8_t opcode) {
    ...
    socket_client->write((uint8_t) (opcode | WS_FIN));
	if (size > 0x7FFF) {
		socket_client->write((uint8_t) (WS_SIZE64 | WS_NOMASK));
        socket_client->write((uint8_t) ((size >> 24) & 0xFF));
        socket_client->write((uint8_t) ((size >> 16) & 0xFF));
        socket_client->write((uint8_t) ((size >> 8) & 0xFF));
        socket_client->write((uint8_t) (size & 0xFF));
	}
    else if (size > 125) {
        socket_client->write((uint8_t) (WS_SIZE16 | WS_NOMASK));
        socket_client->write((uint8_t) ((size >> 8) & 0xFF));
        socket_client->write((uint8_t) (size & 0xFF));
    } 
	else {
        socket_client->write((uint8_t)(((uint8_t) (size & 0xFF)) | WS_NOMASK));
    }
	...
	socket_client->write((uint8_t*)str, size);
     
    // for (int i=0; i<size; ++i) {
    //     socket_client->write((uint8_t)(str[i])); //^ mask[i % 4]
    // }
}
